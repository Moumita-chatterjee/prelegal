...
The current implementation supports all 11 document types via AI chat with full user authentication. Once a document type is resolved, chat history and drafted fields are autosaved to a per-user `documents` table so users can resume or delete a document from `/my-documents`.
...
**Post-review fixes** (same PR, found via user testing): the chat prompt never told the model the current date, so relative expressions like "today" were echoed back as the literal field value instead of being resolved — fixed by injecting today's date into the system prompt plus an explicit instruction to resolve relative dates to ISO format, with a backend backstop (`llm.py::_clear_malformed_dates`) that nulls out any date field that still isn't valid ISO after extraction rather than let a literal "today" reach the rendered document. Similarly, extracted text values (e.g. "new york") weren't being normalized to standard capitalization — fixed with an explicit prompt instruction. A separately reported "can't sign in again after signing out" turned out not to be a code bug: reproduced the full sign-out/sign-in flow successfully against a stable server: the likely cause is the backend's documented behavior of dropping and recreating the database on every process (re)start (see PREL-4), which would silently wipe an account if the server restarted between signup and a later sign-in.

### PREL-7: Support multiple users & final polish

The Jira ticket had no description beyond its title, so scope was confirmed with the user up front: auth/signup/signin already supported multiple users, so "support multiple users" was interpreted as adding per-user document persistence (the gap called out in every prior ticket above); "final polish" was scoped to UI/UX cleanup, error handling/edge cases, and a bug sweep.

- **Backend persistence**: a `Document` SQLAlchemy model (`backend/app/models.py`) — `user_id` FK, `document_type`, `title`, `messages`/`fields` JSON columns, timestamps — plus `backend/app/routers/saved_documents.py` exposing `POST/GET /api/documents` and `GET/PUT/DELETE /api/documents/{id}`, all scoped to `current_user.id` via the existing `get_current_user` dependency. A foreign document id returns 404 (not 403) to avoid leaking existence, matching REST convention (no prior precedent in this codebase since nothing was previously scoped by id). Covered by pytest (`backend/tests/test_saved_documents.py`): auth requirement, unknown-`documentType` rejection, round-tripping messages/fields, list ordering, update, delete, and cross-user isolation (404, not another user's data).
- **Frontend autosave/resume**: `DocumentChat` now takes `initialMessages` (to seed a resumed chat) and reports each completed turn via a single `onTurnComplete` callback rather than three separate ones — `app/page.tsx` computes the next `documentType`/`values`/`messages` synchronously from that one callback and saves immediately (via `lib/documents/api.ts`), rather than deriving the save from a `useEffect` watching state, which the project's stricter React Compiler ESLint rules (`react-hooks/set-state-in-effect`) reject as a derived-state anti-pattern. The saved document's id round-trips through a `?id=` query param (`window.history.replaceState`, no client-side router involved since this is `output: "export"`) so a page refresh resumes the same document; `DocumentChat` is remounted (`key={documentId ?? "new"}`) exactly once, right after the first autosave assigns an id, to pick up the now-correct `initialMessages`. A new `/my-documents` page (`RequireAuth`-gated, linked from `AuthNav`) lists, resumes, and deletes a user's saved documents; a "New document" affordance (previously absent) resets in-page state.
- **Polish**: `RequireAuth`'s and the resume flow's blank-`return null` loading states got a visible "Loading..." message; the chat log gained an `aria-live="polite"` region; `apiClient.ts` now treats a 204 response as success-with-no-body (needed for `DELETE`) and dispatches a `prelegal:unauthorized` event on any 401, which `AuthContext` listens for to sign the user out and let `RequireAuth` redirect to `/login` — covers the previously-unhandled case of an expired JWT mid-session. `DownloadButton`'s stray `bg-slate-900` (inconsistent with every other primary action) was aligned to the shared brand purple.
- Not done: no frontend test framework was introduced (none existed before this ticket either) — validated via a live browser pass instead (signup, multi-turn NDA and CSA chats, autosave, full-reload resume, list/delete, and a second user confirming no cross-user visibility), consistent with how PREL-6's frontend work was validated.
